...
async def api_bootstrap(request):
    ...
    if is_admin:
        ...
        videos = await db_fetchall("SELECT id,share_code,title,category_id,thumb,published,views,required_ads,featured,trending,created_at FROM videos WHERE published=TRUE ORDER BY created_at DESC")
    for v in videos:
        if v.get("share_code"):
...
async def api_unlock_url(request):
    ...
    vid = request.match_info["video_id"]
    settings = await get_settings()
    row = await db_fetchone("SELECT short_url,required_ads FROM videos WHERE id=%s AND published=TRUE", (vid,))
    if not row:
    ...
    await db_execute("INSERT INTO user_video_events(user_id,video_id,event_type,created_at) VALUES(%s,%s,'unlock_granted',%s)", (int(u["id"]), vid, utcnow_sql()))
    return web.json_response({"ok": True, "url": row.get("short_url") or ""})
...
async def api_admin_video_save(request):
    ...
    existing = await db_fetchone("SELECT share_code,thumb,views FROM videos WHERE id=%s", (d["id"],))
    ...
    await db_execute(
        """INSERT INTO videos(id,share_code,video_code,title,category_id,thumb,deep_link,short_url,broadcast_enabled,broadcast_sent,published,views,required_ads,featured,trending,created_at)
           VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,FALSE,%s,%s,%s,%s,%s,%s)
        ...
        (d["id"], share_code, d["video_code"], d["title"], d.get("category_id"), thumb, d.get("deep_link", ""), d["short_url"], broadcast_enabled, published, views, required_ads, featured, trending, utcnow_sql()),
    )
    return web.json_response({"ok": True, "share_code": share_code, "share_link": share_link, "required_ads": required_ads})
...
async def delete_queue_worker():
    ...
    while True:
        try:
            rows = await db_fetchall("SELECT id,chat_id,message_id,retry_count FROM delete_queue WHERE status='pending' AND delete_at<=CURRENT_TIMESTAMP ORDER BY delete_at ASC LIMIT 100")
            for row in rows:
                try:
