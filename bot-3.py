...
async def storage_channel_post(message: Message):
    ...
    await bot.send_message(
        ...
        "এই Video Bot link এখন package-এর delivery target। Mini App Admin Panel-এ Title + Thumbnail + Video Code দিয়ে Publish করুন।\n\n"
...
async def api_admin_video_save(request):
    ...
    existing = await db_fetchone("SELECT share_code,thumb,views,delivery_mode,short_url FROM videos WHERE id=%s", (d["id"],))
    ...
    await db_execute(
        """INSERT INTO videos(id,share_code,video_code,title,category_id,thumb,deep_link,short_url,delivery_mode,broadcast_enabled,broadcast_sent,published,views,required_ads,featured,trending,created_at)
           ...
           thumb=EXCLUDED.thumb,deep_link=EXCLUDED.deep_link,short_url=EXCLUDED.short_url,delivery_mode=EXCLUDED.delivery_mode,
        ...
        (d["id"], share_code, d["video_code"], d["title"], d.get("category_id"), thumb, d.get("deep_link", ""), short_url, delivery_mode, broadcast_enabled, published, views, required_ads, featured, trending, utcnow_sql()),
    )
